...
<div class="section search-result-wrap">
  <div class="container">
    ...
    <div class="row posts-entry">
      <div class="col-lg-8" id="searchResultsContainer">
        ...
        <?php foreach ($AllWikies as $wiki) : ?>
          <div class="blog-entry d-flex blog-entry-search-item">
            ...
            <div>
              <span class="date"><?php echo date('M. dS, Y', strtotime($wiki['created_at'])); ?> &bullet; <a
                  href="#"><?php echo $wiki['category_name']; ?></a></span>
              <h2><a href="wiki"><?php echo $wiki['title']; ?></a></h2>
              <p><?php // Add content here if needed 
                  ?></p>
              <p><a href="wiki?wiki_id=<?php echo $wiki['id'] ?>"
        ...
        <div class="sidebar-box">
          ...
          <ul class="categories">
            <?php foreach ($mostUsedCategories as $cat) : ?>
              <li><a href="#"><?php echo $cat['categoryName']; ?></a></li>
            <?php endforeach; ?>
          </ul>
        </div>
        <!-- END sidebar-box -->
...
<script>
document.getElementById("s").addEventListener("input", async function (event) {
  const searchTerm = event.target.value.toLowerCase();
  console.log("Search Term:", searchTerm);
  await filterWiki(searchTerm);
});
...
async function filterWiki(searchTerm) {
  ...
  <?php foreach ($AllWikies as $wiki) : ?>
    {
      const title = '<?php echo strtolower($wiki['title']); ?>';
      if (title.includes(searchTerm)) {
        console.log("Adding Result:", <?php echo json_encode($wiki); ?>);
        ...
        `;
        wikiEntries.appendChild(resultDiv);
      }
    }
  <?php endforeach; ?>
}
